...
function CompactTradesmanCard({ worker }: { worker: Worker }) {
  // Truncate bio to ~50 characters
  const shortBio = worker.bio 
    ? worker.bio.length > 50 
      ? worker.bio.substring(0, 50) + '...' 
      : worker.bio
    : null;
    ...
    >
      ...
      <div className="relative w-full aspect-square overflow-hidden bg-slate-100">
        ...
        {worker.subscription_plan === 'pro' && (
          <div className="absolute top-2 right-2 flex items-center gap-1 px-2 py-1 bg-amber-500 rounded-md shadow-lg">
        ...
        {worker.is_verified && (
          <div className="absolute bottom-2 left-2 flex items-center gap-1 px-1.5 py-0.5 bg-blue-500 rounded shadow-md">
            <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
            <span className="text-xs font-semibold text-white">Verificat</span>
          </div>
        )}
      </div>
      ...
      <div className="p-3">
        <h3 className="font-bold text-slate-900 text-base mb-1 truncate group-hover:text-blue-600 transition-colors">
          {worker.name}
        </h3>
        ...
        {worker.rating > 0 && (
          <div className="flex items-center gap-1 mt-2">
            ...
            <span className="text-sm font-semibold text-slate-700">{worker.rating.toFixed(1)}</span>
          </div>
        )}
      </div>
    </a>
